...
std::vector<double> algo1Time; //minsat algo
std::vector<double> algo2Time; //approx v1
std::vector<double> algo3Time; //approx v2
std::vector<double> algo1VertexCount; //minsat vc number
std::vector<double> algo2VertexCount; //approx v1
std::vector<double> algo3VertexCount; //approx v2
...
void* callApproxVC2(void *arg){
   ...
   // std::cout<<"Time diff in sec: "<<(endTime.tv_sec-startTime.tv_sec)<<std::endl;
    ...
    double time = (double)((double)(endTime.tv_sec-startTime.tv_sec)*1000000) + (double)((double)(endTime.tv_nsec/1000)-(double)(startTime.tv_nsec/1000));
    algo3Time.push_back(time);
    algo3VertexCount.push_back(listofCovers.size());
...
void* callApproxVC1(void *arg){
   ...
   double time = (double)((double)(endTime.tv_sec-startTime.tv_sec)*1000000) + (double)((double)(endTime.tv_nsec/1000)-(double)(startTime.tv_nsec/1000));
    ...
    algo2Time.push_back(time);
    algo2VertexCount.push_back(listofCovers.size());
...
//            std::cout<<"here"<<std::endl;
        std::set<std::pair<int,int>> tempedgeInputA1 = edgeInputA1;
        int localpp =pp;
            clockid_t cid;
   ...
   // std::cout<<"Time diff in sec: "<<(endTime.tv_sec-startTime.tv_sec)<<std::endl;
    ...
    double time = (double)((double)(endTime.tv_sec-startTime.tv_sec)*1000000) + (double)((double)(endTime.tv_nsec/1000)-(double)(startTime.tv_nsec/1000));
//     std::cout<<"hereinside"<<"  pp:"<<pp<<std::endl;  
    if(time<5000000 && localpp==pp){
     algo1Time.push_back(time);
     algo1VertexCount.push_back(listofCovers.size());
    }else{
        pthread_exit(NULL);
    }
...
void threadCreation(){
    ...
    waitTime.tv_sec+=5;  //having wait time of 2 sec for this graph cnf-sat solver thread
    

    int status=pthread_timedjoin_np(thread1,nullptr,&waitTime);
    ...
    if(status!=0){
       ...
       // pthread_cancel(thread1);
        //std::cout<<"here"<<status<<"  pp:"<<pp<<std::endl;
        algo1Time.push_back(5000000);
        algo1VertexCount.push_back(-1);
    }
    ...
    while (!std::cin.eof()) {
        ...
        try{
            ...
            if(order=='V')
                ...
                // std::cout<<algo1VertexCount.size()<<std::endl;
                // std::cout<<algo1Time.size()<<std::endl;
                // std::cout<<algo2VertexCount.size()<<std::endl;
                // std::cout<<algo2Time.size()<<std::endl;
                // std::cout<<algo3VertexCount.size()<<std::endl;
                // std::cout<<algo3Time.size()<<std::endl;
                
            }


            for(int i=0;i<algo3Time.size();i++){
                std::cout<<algo1Time.at(i)<<","<<algo2Time.at(i)<<","<<algo3Time.at(i)<<","
                <<algo2VertexCount.at(i)<<","<<algo3VertexCount.at(i)<<","<<algo1VertexCount.at(i)<<std::endl;
            }
            edgeInputA1.clear();
            edgeInputA2.clear();
            edgeInputA3.clear();
            ...
            algo2Time.clear();
            algo3Time.clear();
            algo1VertexCount.clear();
            algo2VertexCount.clear();
            algo3VertexCount.clear();
            //CNF_SAT="CNF-SAT-VC: timeout";
